...
const RoundTwoScreen = () => {

  const navigate = useNavigate();
  const [LobbyIdAndRound, setLobbyIdAndRound] = useState(false);
  ...
  //     if (type === "join") {
  //       navigate(
  //         `/round-three?type=join&&round=3&&code=${LobbyIdAndRound?.lobby?.lobbyCode}&&userId=${storedUserId}`
  //       );
  //     } else {
  //       navigate(
  //         `/round-three?round=3&&code=${LobbyIdAndRound?.lobby?.lobbyCode}&&id=${LobbyIdAndRound?.lobby?.lobbyCreator?._id}`
  //       );
  //     }
  //     // navigate(
  //     //   `/round-three?round=3&&code=${LobbyIdAndRound?.lobby?.lobbyCode}&&id=${LobbyIdAndRound?.lobby?.lobbyCreator?._id}`
  ...
  const onFinish = (payload) => {
    if (showAnswer === false) {
      let load = {
        ...
        round: answerData?.rounds[1]?.round,
        loop: "1",
      };
      makeRequest(
        setLoader,
  ...
  const onSuccessAnswerTwo = (res) => {
    setLobbyIdAndRound(res?.lobby?.isAnswersCompleted);
    setRemainingTime(45)
  ...
  return (
    <div>
      <div className="center">
        {/* {showImage && <img src={roundTwo} style={{ width: "100%" }} />} */}
        <Row>
          {/* {!showImage && ( */}
          <div className="form-box">
            <Row justify="center">
              <h1 className="questions">Round 2 - {remainingTime} seconds</h1>
            </Row>
